...
## Create conda environment (Python 3.11.4)
...
.PHONY: data
data:
	$(PYTHON_INTERPRETER) -m src.pipeline.prepare_dataset

## Prepare dataset (overwrite existing files)
.PHONY: data-overwrite
data-overwrite:
	$(PYTHON_INTERPRETER) -m src.pipeline.prepare_dataset --overwrite

## Prepare dataset with verbose output
.PHONY: data-verbose
data-verbose:
	$(PYTHON_INTERPRETER) -m src.pipeline.prepare_dataset --verbose
...
.PHONY: features
features:
	$(PYTHON_INTERPRETER) -m src.pipeline.run_feature_engineering
